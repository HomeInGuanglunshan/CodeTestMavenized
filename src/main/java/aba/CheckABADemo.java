...
public final class CheckABADemo {
	 ...
	 */
	static AtomicStampedReference<String> envelope = new AtomicStampedReference<String>("远方的来信", 0);
	...
	public static void main(String[] args) throws InterruptedException {
		...
		Thread t1 = new Thread(new Runnable() {
			...
			public void run() {
				...
				System.out.println("T1首先看到了信封里的邮件内容[ " + mailContent + " ]（A）。" + "信封上还有一个操作标记：" + mailStamp);
				...
				// T1尝试给‘远方的来信’回信
				boolean result = envelope.compareAndSet(mailContent, "远方的回信", mailStamp, ++mailStamp);

				if (result) {
					System.out.println("\nT1在返回后检查邮件内容和信封上面的操作标记。" + "邮件内容还是一样。信封的操作标记也没人动过。" + "现在可以写回信了[ "
		...
		Thread t2 = new Thread(new Runnable() {
			...
			public void run() {
				...
				boolean firstOpt = envelope.compareAndSet(mailContent, "", mailStamp[0], ++mailStamp[0]);
				if (firstOpt)
					System.out.println("\nT2悄悄将信封里的邮件内容[ " + mailContent + " ]取走（B）。" + "现在邮件内容已经不在信封里了。"
							+ "信封上的操作标记首次被改动了：" + envelope.getStamp());

				// T2第二次修改了信封里的邮件内容和信封上的操作标记
				boolean secondOpt = envelope.compareAndSet("", mailContent, mailStamp[0], ++mailStamp[0]);
				if (secondOpt)
					System.out.println("T2悄悄把信封里的邮件内容[ " + envelope.getReference() + " ]放回（A）。" + "现在邮件内容好像没被动过一样。"
